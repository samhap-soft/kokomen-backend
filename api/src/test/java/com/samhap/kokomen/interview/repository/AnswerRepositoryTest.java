...
class AnswerRepositoryTest extends BaseTest {
    ...
    void 루트질문과_랭크로_상위_답변들을_인터뷰_좋아요순으로_조회한다() {
        ...
        RootQuestion rootQuestion = rootQuestionRepository.save(
                RootQuestionFixtureBuilder.builder().content("자바의 특징은?").questionOrder(10).build());

        // A 랭크 답변들 (인터뷰 좋아요 수: 10, 5, 1)
        Member member1 = memberRepository.save(MemberFixtureBuilder.builder().nickname("김철수").build());
        Interview interview1 = interviewRepository.save(
                InterviewFixtureBuilder.builder().member(member1).rootQuestion(rootQuestion).likeCount(10L).build());
        Question question1 = questionRepository.save(QuestionFixtureBuilder.builder().interview(interview1).build());
        Answer answerA1 = answerRepository.save(
                AnswerFixtureBuilder.builder().question(question1).content("답변1").answerRank(AnswerRank.A).build());

        Member member2 = memberRepository.save(MemberFixtureBuilder.builder().nickname("이영희").build());
        Interview interview2 = interviewRepository.save(
                InterviewFixtureBuilder.builder().member(member2).rootQuestion(rootQuestion).likeCount(5L).build());
        Question question2 = questionRepository.save(QuestionFixtureBuilder.builder().interview(interview2).build());
        Answer answerA2 = answerRepository.save(
                AnswerFixtureBuilder.builder().question(question2).content("답변2").answerRank(AnswerRank.A).build());

        Member member3 = memberRepository.save(MemberFixtureBuilder.builder().nickname("박민수").build());
        Interview interview3 = interviewRepository.save(
                InterviewFixtureBuilder.builder().member(member3).rootQuestion(rootQuestion).likeCount(1L).build());
        Question question3 = questionRepository.save(QuestionFixtureBuilder.builder().interview(interview3).build());
        answerRepository.save(
                AnswerFixtureBuilder.builder().question(question3).content("답변3").answerRank(AnswerRank.A).build());

        // B 랭크 답변 (인터뷰 좋아요 수: 3)
        Member member4 = memberRepository.save(MemberFixtureBuilder.builder().nickname("정수진").build());
        Interview interview4 = interviewRepository.save(
                InterviewFixtureBuilder.builder().member(member4).rootQuestion(rootQuestion).likeCount(3L).build());
        Question question4 = questionRepository.save(QuestionFixtureBuilder.builder().interview(interview4).build());
        answerRepository.save(
                AnswerFixtureBuilder.builder().question(question4).content("답변4").answerRank(AnswerRank.B).build());

        // when - A 랭크 답변 2개 조회
        List<Answer> aRankAnswers = answerRepository.findTopAnswersByRootQuestionAndRank(rootQuestion.getId(),
                AnswerRank.A, -1L, 2);
    ...
    void 특정_인터뷰를_제외하고_답변을_조회한다() {
        ...
        List<Answer> answers = answerRepository.findTopAnswersByRootQuestionAndRank(rootQuestion.getId(), AnswerRank.A,
                interview1.getId(), 10);
    ...
    void 다른_루트질문의_답변은_조회되지_않는다() {
        // given
        RootQuestion rootQuestion1 = rootQuestionRepository.save(
                RootQuestionFixtureBuilder.builder().content("자바의 특징은?").questionOrder(1).build());
        RootQuestion rootQuestion2 = rootQuestionRepository.save(
                RootQuestionFixtureBuilder.builder().content("스프링의 특징은?").questionOrder(2).build());
        ...
        Interview interview1 = interviewRepository.save(
                InterviewFixtureBuilder.builder().member(member1).rootQuestion(rootQuestion1).likeCount(10L).build());
        Question question1 = questionRepository.save(QuestionFixtureBuilder.builder().interview(interview1).build());
        answerRepository.save(
                AnswerFixtureBuilder.builder().question(question1).content("자바답변").answerRank(AnswerRank.A).build());

        // rootQuestion2에 대한 답변
        Interview interview2 = interviewRepository.save(
                InterviewFixtureBuilder.builder().member(member2).rootQuestion(rootQuestion2).likeCount(5L).build());
        Question question2 = questionRepository.save(QuestionFixtureBuilder.builder().interview(interview2).build());
        Answer answer2 = answerRepository.save(
                AnswerFixtureBuilder.builder().question(question2).content("스프링답변").answerRank(AnswerRank.A).build());

        // when - rootQuestion2에 대한 답변만 조회
        List<Answer> answers = answerRepository.findTopAnswersByRootQuestionAndRank(rootQuestion2.getId(), AnswerRank.A,
                -1L, 10);

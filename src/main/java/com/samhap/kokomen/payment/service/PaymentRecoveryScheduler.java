...
public class PaymentRecoveryScheduler {
    ...
    @Transactional
    protected void processRecovery(TosspaymentsPayment payment) {
        String paymentKey = payment.getPaymentKey();

        TosspaymentsPayment freshPayment = tosspaymentsPaymentService.readByPaymentKey(paymentKey);
        if (freshPayment.isTerminal()) {
            log.info("복구 스킵 - 이미 종료 상태: paymentKey={}, state={}", paymentKey, freshPayment.getState());
            return;
        }
        ...
        log.info("복구 진행 - paymentKey={}, 내부상태={}, 토스상태={}", paymentKey, freshPayment.getState(), tossStatus);

        switch (tossStatus) {
            case DONE -> recoverAsDone(freshPayment, tossResponse);
            case CANCELED, PARTIAL_CANCELED -> {
                freshPayment.updateState(PaymentState.CANCELED);
                log.info("복구 완료(취소) - paymentKey={}", paymentKey);
            }
            case EXPIRED, ABORTED -> {
                freshPayment.updateState(PaymentState.NOT_NEED_CANCEL);

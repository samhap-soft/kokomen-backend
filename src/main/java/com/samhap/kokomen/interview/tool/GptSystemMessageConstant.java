...
public final class GptSystemMessageConstant {
    ...
    public static final String END_SYSTEM_MESSAGE = """
            ...
            <output>
            반드시 제공된 함수(generate_total_feedback)를 호출하여 다음 네 필드를 함께 제출하라.
            - reasoning : last_answer_analysis(가장 최근 답변에 대한 rubric 평가 근거)와 overall_summary(전체 면접의 강점/개선/학습 방향 정리)를 한 단락으로 작성. 사용자에게 노출되지 않음.
            - rank : 가장 최근 답변에 대한 랭크. A, B, C, D, F 중 한 글자. 전체 답변 누적이 아닌 가장 최근 답변만을 기준으로 평가한다.
            - feedback : 가장 최근 답변에 대한 3-4문장 피드백. answer_rank에 맞는 톤. 존댓말, 점수/랭크 미언급, 개행 없이 한 단락.
            - overall_summary : 전체 면접에 대한 종합 평가. 다음 3개 서브필드로 분리해 작성한다.

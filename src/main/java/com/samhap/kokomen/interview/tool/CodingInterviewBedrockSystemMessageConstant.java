...
public final class CodingInterviewBedrockSystemMessageConstant {

    public static final String CODING_IN_PROGRESS_RANK_AND_NEXT_QUESTION_PROMPT = """
            <role>
            %s
            </role>
            ...
            가장 최근 답변(가장 마지막 user 메시지)만 평가하고, 그 코드를 토대로 다음 꼬리 질문 한 개를 생성하라.
            </task>
    ...
    public static final String CODING_END_PROMPT = """
            <role>
            %s
            </role>
            ...
            가장 최근 답변에 대한 rank와 feedback, 그리고 면접 전체에 대한 종합 평가를 overall_summary 객체로 작성하라.
            </task>
            ...
            <output>
            제공된 도구를 호출해 reasoning, rank, feedback, overall_summary 네 필드를 함께 제출하라.
            - reasoning : last_answer_analysis(가장 최근 답변에 대한 rubric 평가 근거)와 overall_summary(전체 코딩 테스트의 강점/개선/학습 방향 정리)를 한 단락으로 작성. 사용자에게 노출되지 않음.
            - rank : 가장 최근 답변에 대한 랭크. A, B, C, D, F 중 한 글자. 전체 답변 누적이 아닌 가장 최근 답변만을 기준으로 평가한다.
            - feedback : 가장 최근 답변에 대한 3-4문장의 정중한 피드백. 자연스러운 산문으로 작성하되, 코드를 포함할 경우 반드시 마크다운 코드 블록(```)으로 감싸서 작성한다. 코드 외 설명은 한 단락으로 이어서 작성하고, 존댓말을 사용하며 점수/랭크는 언급하지 않는다.
            - overall_summary : 전체 코딩 테스트에 대한 종합 평가. 다음 3개 서브필드로 분리해 작성한다.
              ...
              세 서브필드는 서버에서 한 단락으로 합성되므로 각 항목은 독립적인 한두 문장으로 자연스럽게 이어지게 작성한다. 인사·점수·랭크 언급 금지.
            </output>
            """.formatted(
            CodingInterviewPromptFragments.PERSONA,
            CodingInterviewPromptFragments.SECURITY_RULES,
    ...
    public static final String CODING_ANSWER_FEEDBACK_PROMPT = """
            <role>
            %s
            </role>

            <task>
            아래는 라이브 코딩 테스트의 대화 흐름이며, 가장 최근 답변에 매겨진 answer_rank는 system context 영역에 별도로 제공된다.
            너의 작업은 가장 최근 질문에 대해 면접자가 제출한 코드 또는 설명에 대한 피드백을 작성하는 것이다.
            </task>
            ...
            코드 정확성, 시간·공간 복잡도, 엣지 케이스 처리, 가독성 및 구조
            </evaluation_criteria>
            ...
            피드백은 자연스러운 산문으로 작성하되, 코드를 포함할 경우 반드시 마크다운 코드 블록(```)으로 감싸서 작성한다. 코드 외 설명은 존댓말로 한 단락으로 이어서 작성한다.
            </output>
            """.formatted(
            CodingInterviewPromptFragments.PERSONA,
            CodingInterviewPromptFragments.SECURITY_RULES,
    ...
    private CodingInterviewBedrockSystemMessageConstant() {
    }
}

...
public class ResumeAnalysisFacadeService {
     ...
     * 기존 회원에게도 무료 1회가 다시 부여된다.
     *
     * <p>이 재부여는 <b>의도적으로 수용된 결과다</b> — 발견되지 않은 버그가 아니다. 구 이력을 보존해 판정에
     * 쓰는 방법(퍼지 전에 유료 사용 회원 id를 별도 테이블로 스냅샷)을 검토했으나, 그 테이블을 영구히
     * 유지하는 비용이 재부여되는 무료 사용분보다 크다고 판단해 채택하지 않았다. 구 이력은 이미 삭제됐으므로
     * 이 판정을 되돌릴 수단은 남아 있지 않다. 따라서 "기존 회원이 무료 1회를 다시 받는다"를 결함으로 보고
     * 판정 근거를 늘리려 하지 말 것.
     *

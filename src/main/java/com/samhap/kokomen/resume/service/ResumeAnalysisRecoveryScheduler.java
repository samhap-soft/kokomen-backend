 ...
 * 훑는 낭비를 줄이는 장치다. 그래서 회차가 끝나면 성공이든 실패든 {@code finally}에서 해제한다 — 해제하지
 * 않으면 4분 TTL이 5분 주기의 대부분을 덮어 다른 인스턴스가 사실상 매 회차를 건너뛰고, 실행 중 급사한
 * 인스턴스가 다음 회차까지 키를 쥔다. TTL은 그 급사에 대비한 상한으로만 남는다. 해제는 Lua CAS인
 * {@code releaseLockSafely}로 해서, 자기 락이 TTL로 이미 만료된 뒤라면 다른 인스턴스가 새로 건 락을
 * 지우지 않는다.
 *

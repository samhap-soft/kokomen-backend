CREATE TABLE tosspayments_payment (
    ...
    state ENUM('NEED_APPROVE', 'APPROVED', 'NOT_NEED_CANCEL', 'NEED_CANCEL',
               'CONNECTION_TIMEOUT', 'CANCELED', 'CLIENT_BAD_REQUEST',
               'SERVER_BAD_REQUEST', 'COMPLETED') NOT NULL,
    service_type ENUM('INTERVIEW') NOT NULL,
...
CREATE TABLE tosspayments_payment_result (
    ...
    tosspayments_status ENUM('READY', 'IN_PROGRESS', 'WAITING_FOR_DEPOSIT', 'DONE',
                             'CANCELED', 'PARTIAL_CANCELED', 'ABORTED', 'EXPIRED') NOT NULL,
    ...
    updated_at DATETIME(6) NOT NULL,
    PRIMARY KEY (id),
    CONSTRAINT fk_payment_result_payment
        FOREIGN KEY (tosspayments_payment_id) REFERENCES tosspayments_payment(id)
);

...
class InterviewMessagesFactoryTest {
    ...
    @Test
    void 이력서_기반_면접_진행을_위해_GPT에게_보낼_메시지는_기본_시스템_프롬프트를_사용한다() {
        // given
        Interview interview = InterviewFixtureBuilder.builder()
                .interviewType(InterviewType.RESUME_BASED)
                .build();
        Question question = QuestionFixtureBuilder.builder().id(1L).content("이력서 기반 질문").build();
        QuestionAndAnswers questionAndAnswers = new QuestionAndAnswers(List.of(question), List.of(), "제 경험은",
                question.getId(), interview);
    ...
    @Test
    void 이력서_기반_면접_종료를_위해_GPT에게_보낼_메시지는_기본_시스템_프롬프트를_사용한다() {
        // given
        Interview interview = InterviewFixtureBuilder.builder()
                .interviewType(InterviewType.RESUME_BASED)
                .build();
